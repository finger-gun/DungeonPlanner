...
The system SHALL keep outdoor ground texture paint mapped to the sculpted outdoor terrain surface so texture-painted areas remain visually aligned after terrain elevation changes, including face-aware rendering on stepped terrain.
...
#### Scenario: Grass paint uses face-aware rendering on stepped terrain
- **WHEN** a stepped outdoor area is painted with grass texture
- **THEN** top faces use grass while side faces use dirt with a soft blend near step tops
- **AND** editing/erasing the paint updates both top and side rendering consistently
