...
- [ ] 1.1 Create a new local `app/` workspace package for the signed-in user experience without changing `web/`, `docs-web/`, or the current anonymous demo/editor behavior.
- [ ] 1.2 Wire development and build entry points for the new authenticated app surface so it can run locally alongside existing repo surfaces.
- [ ] 1.3 Add a minimal authenticated app shell with navigation placeholders for account-aware product features.

## 2. Convex foundation and auth baseline

- [ ] 2.1 Add local Convex development setup, dependencies, and environment wiring for the authenticated app workspace package.
- [ ] 2.2 Create the initial Convex schema and functions for users, role assignments, dungeon records, session records, character records, and pack registry records.
- [ ] 2.3 Implement sign-up, sign-in, sign-out, and authenticated session hydration in the frontend using Convex Auth.
- [ ] 2.4 Add shared auth helpers so protected frontend and backend paths can resolve the current authenticated user identity.

## 3. Role-aware authorization

- [ ] 3.1 Implement additive role storage and lookup for `admin`, `dm`, and `player`.
- [ ] 3.2 Add authorization guards for admin-only and DM-only operations in backend-facing flows.
- [ ] 3.3 Update UI affordances so role-restricted actions are only exposed to authorized users.
