...
type BackendAuthFlow = 'signIn' | 'signUp'
...
function serializeCookie(host: string, name: string, value: string, maxAgeSeconds?: number) {
  const parts = [
...
function buildAuthCookieHeaders(host: string, tokens: ConvexAuthTokens | null) {
...
export function applyAuthCookies(response: Response, request: Request, tokens: ConvexAuthTokens | null) {
  response.setHeader('Set-Cookie', buildAuthCookieHeaders(request.headers.host ?? '', tokens))
}

function isAuthFlow(value: unknown): value is BackendAuthFlow {
  return value === 'signIn' || value === 'signUp'
}

function validateSignInBody(body: unknown): BackendAuthSignInRequest {
  const request = body as Partial<BackendAuthSignInRequest> | null
...
async function runAuthSignIn(args: Record<string, unknown>) {
  return createConvexClient().action('auth:signIn' as never, args as never) as Promise<ConvexSignInResult>
}

async function runAuthSignOut(token: string) {
  await createConvexClient(token).action('auth:signOut' as never, {} as never)
}

async function validateViewerToken(token: string) {
  await createConvexClient(token).query('users:viewer' as never, {} as never)
}
...
async function ensureAccessToken(
  ...
  token: string | null
  cookiesToWrite?: ConvexAuthTokens | null
}> {
...
async function resolveSession(cookies: AuthCookieState) {
...
function sendAuthError(response: Response, error: unknown, status = 400) {
  response.status(status).json({
    error: error instanceof Error ? error.message : 'Authentication request failed.',
  })
}

export function registerAuthFacadeRoutes(app: Express) {
  ...
  app.post('/api/auth/sign-in', async (request, response) => {
    let signInRequest: BackendAuthSignInRequest
  ...
  app.post('/api/auth/token', async (request, response) => {
    const body = (request.body ?? {}) as { forceRefreshToken?: unknown }
    const cookies = readAuthCookies(request)
