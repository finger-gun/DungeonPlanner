...
export async function listGeneratedCharacterModels(config: GeneratedCharacterImageConfig = {}) {
  ...
  const payload = (await readOllamaPayloads(response)).payloads.at(-1) as OllamaTagsResponse | undefined
...
function extractGeneratedImageFromPayloads(payloads: OllamaPayload[]) {
...
async function readOllamaError(response: Response, model = DEFAULT_OLLAMA_IMAGE_MODEL) {
...
async function readOllamaPayloads(response: Response) {
...
function parseOllamaPayloads(rawText: string) {
  const trimmedText = rawText.trim()

  if (!trimmedText) {
    return [] as OllamaPayload[]
  }
...
function buildMissingImagePayloadMessage(
  model: string,
  payloads: OllamaPayload[],
  rawText: string,
) {
...
function normalizeOllamaErrorMessage(message: string, model = DEFAULT_OLLAMA_IMAGE_MODEL) {
  const trimmedMessage = message.trim()

  if (
    /unexpected end of json input/i.test(trimmedMessage)
...
function buildOllamaCompatibilityMessage(model: string) {
...
function isOllamaPayload(value: unknown): value is OllamaPayload {
